...
def get_user(
    ...
    if str(current_user.id) != user_id and current_user.official_id != user_id and current_user.role != UserRole.OFFICER:
...
def deactivate_user(
    ...
    """
    Officer Account Management: 'Delete Account' deactivates user login (is_active=False).
    CRITICAL: DOES NOT DELETE farms, incidents, evidence, inspections, or historical records!
    """
