...
class CorrectiveActionService:
    ...
    def submit_evidence(
        ...
        # Confirmation goes to whoever actually submitted it.
        if user is not None:
            NotificationService.create(
                db,
    ...
    def _check_compliance_closure(db: Session, farm_id: str) -> None:
        # NOTE: this is an internal step called after the caller has already
        # passed authorization for `farm_id` via verify_action(). We fetch the
        # Farm row directly (no `user`) rather than through
        # FarmService.get_farm(), which would raise ForbiddenError for a None
        # user — that bug used to make this crash whenever the last open
        # corrective action on a farm was closed.
        open_actions = (
            db.query(CorrectiveAction)
            .filter(
