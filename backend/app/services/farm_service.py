...
class FarmService:
    ...
    def list_farms(db: Session, user: User | None = None) -> list[Farm]:
        """
        Returns exactly the farms `user` is authorized to access — the same
        scope enforced by `ensure_farm_access` below. This MUST stay in sync
        with `ensure_farm_access`: the farm selector should never offer a
        farm that a later request to that farm would then reject.
        """
        if user is None:
            return []
        ...
        if user.role == UserRole.FARMER:
            ...
            if not allowed:
                return []
            query = query.filter(Farm.id.in_(allowed))

        elif user.role == UserRole.VETERINARIAN:
            assigned = {a.farm_id for a in user.farm_assignments}
            if assigned:
                # Vet with explicit assignments → scoped to exactly those farms.
                ...
                # Vet with neither → matches ensure_farm_access, which blocks all.
                return []

        elif user.role == UserRole.OFFICER:
            if user.district_id:
                query = query.filter(Farm.district_id == user.district_id)
            # Officer with no district_id → national scope, no extra filter
            # (matches ensure_farm_access, which imposes no restriction here).
