...
class GisService:
    ...
    def get_map_nodes(
        ...
        farms = query.all()
        insp_sub = (
            db.query(Inspection.farm_id, func.max(Inspection.inspection_date).label("last_insp"))
            .filter(Inspection.status == InspectionStatus.COMPLETED)
            .group_by(Inspection.farm_id)
            .all()
        )
        insp_map = {farm_id: last_insp.isoformat() if last_insp else "N/A" for farm_id, last_insp in insp_sub}
        ...
        for farm in farms:
            nodes.append(
                {
                    "farm": farm,
                    "last_inspection": insp_map.get(farm.id, "N/A"),
                }
            )
    ...
    def spatial_risk(db: Session, farm_id: str, radius_km: float = 15.0, user: User | None = None) -> dict:
        ...
        inc_counts = (
            db.query(Incident.farm_id, func.count(Incident.id).label("cnt"))
            .filter(
                Incident.status.in_([
            ...
            .group_by(Incident.farm_id)
            .all()
        )
        inc_map = {farm_id: cnt for farm_id, cnt in inc_counts}
...
class InspectionService:
    ...
    def schedule(db: Session, payload: InspectionCreate, user: User) -> Inspection:
        ...
        db.add(inspection)
        NotificationService.notify_farm_farmers(
            db,
            farm,
