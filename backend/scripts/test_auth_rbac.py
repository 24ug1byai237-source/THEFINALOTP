...
def run_tests():
    ...
    # 2. Login as Farmer via password and OTP
    ...
    with Session(engine) as test_db_session:
        # NOTE: OTPRequest.phone must be stored in the same NORMALIZED form
        # that AuthService._normalize_phone() produces (no spaces), since
        # verify_otp() looks up by the normalized value. "+91 9876543210"
        # (as typed by a user) normalizes to "+919876543210".
        test_otp = OTPRequest(
            phone="+919876543210",
            code="854219",
            used=False,
