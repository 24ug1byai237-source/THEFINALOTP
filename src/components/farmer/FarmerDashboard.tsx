...
import { FarmerVetSelectionModal } from "./FarmerVetSelectionModal";
...
export const FarmerDashboard: React.FC<FarmerDashboardProps> = ({
  ...
  const [assignedVetName, setAssignedVetName] = useState<string | null>(
    typeof window !== "undefined" ? localStorage.getItem("assigned_vet_name") : null
  );
  const [assignedVetOfficialId, setAssignedVetOfficialId] = useState<string | null>(
    typeof window !== "undefined" ? localStorage.getItem("assigned_vet_official_id") : null
  );
  ...
  📍         {translateData(activeFarm.location, locale)}
        ...
        <div className="header-action-buttons">
          ...
          <button className="btn-secondary-passport" onClick={() => setIsVetModalOpen(true)}>
            <Stethoscope size={18} />
            <span>{assignedVetName ? `Vet: ${assignedVetName}` : "Select Veterinarian"}</span>
          </button>
      ...
      <FarmerVetSelectionModal
        isOpen={isVetModalOpen}
        onClose={() => setIsVetModalOpen(false)}
        onSelectVet={(vet) => {
          setAssignedVetName(vet.fullName);
          setAssignedVetOfficialId(vet.officialId || vet.id);
        }}
        currentSelectedVetId={assignedVetOfficialId}
      />
