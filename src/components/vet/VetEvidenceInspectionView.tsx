...
import { analyzeEvidenceLocally, isVeterinaryActionPlan, normalizeEvidenceAnalysis, relevanceBadge, stripVetPlanMarker } from "../../utils/evidenceAnalysis";
...
export const VetEvidenceInspectionView: React.FC = () => {
  const { activeFarm, allFarms, refreshFarms } = useAuth();
  const { refreshNotifications } = useNotifications();
  const [actions, setActions] = useState<CorrectiveAction[]>([]);
  const [loading, setLoading] = useState(true);
  const [loadError, setLoadError] = useState<string | null>(null);
  const [farmFilter, setFarmFilter] = useState<string>(activeFarm.id);
  ...
  const load = useCallback(async (options?: { silent?: boolean }) => {
    ...
    setLoadError(null);
    try {
      const targetFarmId = farmFilter === "all" ? undefined : farmFilter;
      let list = await correctiveActionService.getAwaitingVerification(targetFarmId);
      list = list.filter(
        (a) =>
      ...
      const msg = err instanceof Error ? err.message : "Failed to load evidence queue.";
      setLoadError(msg);
      setActions([]);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    load();
    // Poll silently every 30 s (reduced from 12 s to cut backend load).
    const timer = window.setInterval(() => load({ silent: true }), 30_000);
    return () => {
  ...
  useEffect(() => {
    ...
    if (selected.evidenceAnalysis) {
      setAnalysis(normalizeEvidenceAnalysis(selected.evidenceAnalysis));
      return;
    }
    let cancelled = false;
  ...
  const handleVerify = async (actionType: "confirm" | "reject" | "request_more", noteOverride?: string) => {
    if (!selected) return;
    const approved = actionType === "confirm";
    const note = noteOverride ?? vetNote;
    setProcessing(true);
    setMessage("");
    try {
      await correctiveActionService.verifyAction(selected.id, approved, note || undefined, actionType);
      if (selected.farmId) {
      ...
      setMessage(
        actionType === "confirm"
          ...
          ? "More evidence requested. Farmer notified."
          : "Evidence rejected. Farmer notified."
      );
      await load();
    } catch (err) {
  ...
  return (
    <div className="vet-evidence-view">
      <div className="vet-header-card">
        ...
        <div className="vet-status-summary" style={{ display: "flex", gap: "12px", alignItems: "center" }}>
          <select
            value={farmFilter}
          ...
          >
            <option value="all">All District Farms ({allFarms.length})</option>
            {allFarms.map((farm) => (
      ...
      {loading && actions.length === 0 ? (
        ...
        <div className="empty-state evidence-empty-state">
          <FileSearch size={40} />
          <p>Failed to load evidence queue.</p>
          <p className="text-muted">{loadError}</p>
          <button type="button" className="btn-secondary" style={{ marginTop: "12px" }} onClick={() => load()}>
            Retry
          </button>
        </div>
      ) : actions.length === 0 ? (
        <div className="empty-state evidence-empty-state">
          <FileSearch size={40} />
          <p>No evidence pending for this farm.</p>
          <p className="text-muted">
            {farmFilter === "all"
              ? "No corrective-action evidence is currently awaiting inspection across any district farm."
              : `No corrective-action evidence awaiting inspection for ${allFarms.find(f => f.id === farmFilter)?.name ?? farmFilter}.`}
          </p>
        </div>
      ) : (
