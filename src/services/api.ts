...
import { analyzeEvidenceLocally, isVeterinaryActionPlan, normalizeEvidenceAnalysis, VET_PLAN_MARKER } from "../utils/evidenceAnalysis";
...
export const farmService = {
  ...
  async getAllFarms(options?: { force?: boolean }): Promise<Farm[]> {
    const raw = await cachedApiGet<unknown>("/farms", options);
    const farms: Farm[] = Array.isArray(raw) ? (raw as Farm[]) : [];
    const deduped = dedupeById(farms);
    scheduleIdle(() => {
      Promise.all(
        deduped.map((farm) => cacheFarmBundle(farm.id, { farm }).catch(() => undefined))
      ).catch(() => undefined);
    });
    return deduped;
  },
...
export const incidentService = {
  async getIncidents(farmId?: string, options?: { force?: boolean }): Promise<IncidentReport[]> {
    ...
    const raw = await cachedApiGet<unknown>(path, options);
    const incidents: IncidentReport[] = Array.isArray(raw) ? (raw as IncidentReport[]) : [];
    const deduped = dedupeById(incidents);
    if (farmId) {
      scheduleIdle(() => {
        import("../offline/storage/cacheStore")
          .then((m) => m.getCachedFarmBundle(farmId))
          .then((cached) => {
            if (cached) {
              return cacheFarmBundle(farmId, { farm: cached.farm, incidents: deduped });
            }
          })
          .catch(() => undefined);
      });
...
export const correctiveActionService = {
  attachEvidenceAnalysis(action: CorrectiveAction): CorrectiveAction {
    if (action.evidenceAnalysis) {
      return { ...action, evidenceAnalysis: normalizeEvidenceAnalysis(action.evidenceAnalysis) };
    }
    if (!action.submittedEvidence) return action;
  ...
  async getActions(farmId?: string, options?: { force?: boolean }): Promise<CorrectiveAction[]> {
    ...
    const list = raw as CorrectiveAction[];
    ...
    if (farmId) {
      scheduleIdle(() => {
        import("../offline/storage/cacheStore")
          ...
          .then((cached) => {
            if (cached) {
              return cacheFarmBundle(farmId, { farm: cached.farm, correctiveActions: deduped });
            }
          })
          .catch(() => undefined);
      });
  ...
  async analyzeEvidence(actionId: string): Promise<EvidenceAnalysis> {
    try {
      const result = await cachedApiGet<EvidenceAnalysis>(`/corrective-actions/${actionId}/analyze-evidence`);
      return normalizeEvidenceAnalysis(result);
    } catch (err) {
      if (!isNotFoundError(err)) throw err;
      const all = await correctiveActionService.getActions();
  ...
  async verifyAction(
    ...
    approved: boolean,
    notes?: string,
    actionType?: "confirm" | "reject" | "request_more"
  ): Promise<CorrectiveAction> {
    const result = await apiFetch<CorrectiveAction>(`/corrective-actions/${actionId}/verify`, {
      method: "POST",
      body: JSON.stringify({ approved, notes, actionType }),
    });
    invalidateApiCache("/corrective-actions");
    invalidateApiCache("/farms");
    invalidateApiCache("/notifications");
  ...
  async getAwaitingVerification(farmId?: string, options?: { force?: boolean }): Promise<CorrectiveAction[]> {
    const query = farmId ? `?farmId=${encodeURIComponent(farmId)}` : "";
    try {
      const list = await cachedApiGet<CorrectiveAction[]>(`/corrective-actions/awaiting-verification${query}`, options);
      if (!Array.isArray(list)) {
        return [];
      }
      const filtered = farmId ? list.filter((a) => a.farmId === farmId) : list;
      return dedupeById(
        correctiveActionService.sortEvidenceQueue(
          filtered.map((a) => correctiveActionService.attachEvidenceAnalysis(a))
        )
      );
    } catch (err) {
      if (!isNotFoundError(err)) return [];
      const all = await correctiveActionService.getActions(farmId);
      return dedupeById(
        correctiveActionService.sortEvidenceQueue(
          all
            .filter(
...
export const officerService = {
  ...
  async scheduleInspection(
    ...
    const result = await apiFetch<{ id: string }>("/officer/inspections", {
      method: "POST",
      body: JSON.stringify({ farmId, scheduledAt, notes }),
    });
    invalidateApiCache("/officer");
    return result;
  },
...
export const notificationService = {
  async getNotifications(role?: UserRole, options?: { force?: boolean }): Promise<NotificationItem[]> {
    ...
    const raw = await cachedApiGet<unknown>(`/notifications${query}`, {
      ...options,
      ttlMs: 10_000,
      swr: false,
    });
...
export const userService = {
  async getVeterinarians(): Promise<ApiUserResponse[]> {
    return apiFetch<ApiUserResponse[]>("/users/veterinarians");
  },
  ...
  async deleteUser(userId: string): Promise<ApiUserResponse> {
    return apiFetch<ApiUserResponse>(`/users/${userId}`, {
      method: "DELETE",
    });
  },
};
